...
export class ActualizarUserComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    this.userService.obtenerUsuarioPorId(this.id).pipe(
      tap(dato => {
        this.user = dato;
      }),
      catchError(error => {
  ...
  irAlaListaDeUsuarios(): void {
    ...
    Swal.fire('Usuario actualizado', `El usaurio ${this.user.name} ha sido actualizado con éxito`, 'success');
  }

  onSubmit(): void {
    this.userService.actualizarUsuario(this.id, this.user).pipe(
      tap(() => {
