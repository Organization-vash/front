...
import { DocumentType } from '../../../shared/models/document-type.enum';
...
export class FormularioCgComponent {
  ...
  validarFormulario() {
    ...
    this.customerService
      .validarUsuario(this.nombre, documentType, docNumber)
      .subscribe({
        next: (data) => {
          if (data.message) {
  ...
  generarCodigo() {
    const service = this.servicios.find(
      (s) => s.name === this.servicioSeleccionado
    );
    if (service) {
      ...
      this.ticketService
        .generarTicket(this.numeroDocumento, this.nombre, serviceId, agencyId)
        .subscribe({
          next: (response) => {
